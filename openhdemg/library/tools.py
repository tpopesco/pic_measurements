...
def compute_svr(
    ...
    """
    Fit MU discharge rates with Support Vector Regression (SVR).

    (docstring unchanged, abbreviated here …)
    """

    idr = compute_idr(emgfile)           # instantaneous DR for every MU
    svrfit_acm, svrtime_acm, gensvr_acm = [], [], []

    for mu in range(len(idr)):
        # ------------------------------------------------------------------
        # 0.  no pulses at all  →  just return a NaN-filled vector
        # ------------------------------------------------------------------
        if idr[mu].size == 0:
        ...
        # ------------------------------------------------------------------
        # 1.  build X / y  (remove the first discharge = NaN diff)
        # ------------------------------------------------------------------
        xtmp = idr[mu].timesec[1:].to_numpy().reshape(-1, 1)
        ytmp = idr[mu].idr[1:].to_numpy()

        # ------------------------------------------------------------------
        # 2.  drop rows where y is NaN  (keeps arrays aligned)
        # ------------------------------------------------------------------
        valid = ~np.isnan(ytmp)
        xtmp = xtmp[valid]
        ytmp = ytmp[valid]

        # too little data → give up on this MU
        if ytmp.size < 3:
            gensvr_acm.append(np.nan * np.ones(emgfile["EMG_LENGTH"]))
            svrfit_acm.append([])
            svrtime_acm.append([])
        ...
        # ------------------------------------------------------------------
        # 3.  sample-weight vector  (end-point emphasiser)
        # ------------------------------------------------------------------
        smpwht = np.ones(len(ytmp))
        smpwht[: endpointweights_numpulses] *= endpointweights_magnitude
        smpwht[-endpointweights_numpulses :] *= endpointweights_magnitude
        ...
        # ------------------------------------------------------------------
        eps_iqr = iqr(ytmp)
        epsilon = 0.1 if (np.isnan(eps_iqr) or eps_iqr == 0) else eps_iqr / 11
        ...
        try:
            svr = SVR(
                kernel="rbf",
                gamma=float(gammain),
                C=float(abs(regparam)),
                epsilon=float(epsilon),
            )
        except ValueError:
            # gamma or C not finite – treat MU as unusable
            gensvr_acm.append(np.nan * np.ones(emgfile["EMG_LENGTH"]))
            svrfit_acm.append([])
            svrtime_acm.append([])
            continue
        ...
        try:
            ...
            # something still wrong with the data
            gensvr_acm.append(np.nan * np.ones(emgfile["EMG_LENGTH"]))
            svrfit_acm.append([])
            svrtime_acm.append([])
            continue
        ...
        # ------------------------------------------------------------------
        # 6.  create a 1-sample-resolution prediction between first & last pulse
        #     (unchanged logic from the original implementation)
        # ------------------------------------------------------------------
        mup_samples = idr[mu].mupulses.to_numpy()
        mup_samples = mup_samples[~np.isnan(mup_samples)].astype(int)

        pred_samples = np.arange(mup_samples[0], mup_samples[-1] + 1)
        pred_time = (pred_samples / emgfile["FSAMP"]).reshape(-1, 1)

        smoothfit = svr.predict(pred_time)
        gen_svr = np.nan * np.ones(emgfile["EMG_LENGTH"])
        gen_svr[pred_samples] = smoothfit

        # store
        svrfit_acm.append(smoothfit.copy())
        svrtime_acm.append(pred_time.squeeze().copy())
